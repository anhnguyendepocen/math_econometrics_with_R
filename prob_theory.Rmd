...
This document offers a brief introduction to probability theory.  Econometrics is the machinery of statistical inference deployed to extract information out of observational data sets.  Statistical inference takes data and extracts from it information or knowledge.  In order to do so, statistical inference relies heavily on a *mathematical theory* of learning known as *probability theory*.  To learn or to acquire knowledge means to reduce one's relative ignorance or uncertainty about the world.  Thus, probability theory is a set of theoretical elements that help us to organize our understanding of the part of the world we do not yet understand or know. 

Make sure that the elementary principles of probability theory are clear in your mind, because everything we will do in the subsequent sections of the course is based on these principles.

But before we get to probability theory, we will cover briefly the summation operator.  Operators are symbolic commands that tell us what procedure to apply to other symbols or numbers.  Think of an operator as a sentence in common language that tells you what to do next.  Once we understand clearly what the operator means, all we need to do is mechanically apply the command issued by it.
...
Example:
$$\sum_{i=1}^2 (x_i + y_i) = (x_1 + y_1) + (x_2 + y_2) = x_1 + y_1 + x_2 + y_2$$
$$= x_1 + x_2 + y_1 + y_2 = (x_1 + x_2) + (y_1 + y_2) = \sum_{i=1}^2 x_i + \sum_{i=1}^2 y_i.$$
...
Example:
$$\sum_{i=1}^3 (4 + 5 x_i) = \sum_{i=1}^3 4 + \sum_{i=1}^3 5 x_i = (3 \times 4) + 5 \sum_{i=1}^3 x_i = 12 + 5 \sum_{i=1}^3 x_i.$$
...
In econometrics, we often deal with data tables.  The following property of the summation operator simply says that the total sum is the same whether one adds up first the numbers along the rows first and then adds the partial sums or one adds up first the numbers along the columns and then sums up the partial results.  A property of the double summation operator is that the summations are interchangeable:
